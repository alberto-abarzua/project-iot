...
static const char *TAG_C = "example_connect";
...
static void example_handler_on_wifi_disconnect(void *arg,
    ...
    if (s_retry_num > CONFIG_EXAMPLE_WIFI_CONN_MAX_RETRY) {
        ESP_LOGI(TAG_C, "WiFi Connect failed %d times, stop reconnect.",
                 s_retry_num);
    ...
    ESP_LOGI(TAG_C, "Wi-Fi disconnected, trying to reconnect...");
...
static void example_handler_on_sta_got_ip(void *arg,
    ...
    ESP_LOGI(TAG_C, "Got IPv4 event: Interface \"%s\" address: " IPSTR,
             esp_netif_get_desc(event->esp_netif), IP2STR(&event->ip_info.ip));
    if (s_semph_get_ip_addrs) {
        xSemaphoreGive(s_semph_get_ip_addrs);
    } else {
        ESP_LOGI(TAG_C, "- IPv4 address: " IPSTR ",",
...
static void example_handler_on_sta_got_ipv6(void *arg,
    ...
    if (ipv6_type == EXAMPLE_CONNECT_PREFERRED_IPV6_TYPE) {
        if (s_semph_get_ip6_addrs) {
            ...
            ESP_LOGI(TAG_C, "- IPv6 address: " IPV6STR ", type: %s",
...
esp_err_t example_wifi_sta_do_connect(wifi_config_t wifi_config, bool wait) {
    ...
    ESP_LOGI(TAG_C, "Connecting to %s... using %s", wifi_config.sta.ssid,wifi_config.sta.password);
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
    esp_err_t ret = esp_wifi_connect();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_C, "WiFi connect failed! ret:%x", ret);
        return ret;
    }
    if (wait) {
        ESP_LOGI(TAG_C, "Waiting for IP(s)");
...
esp_err_t example_wifi_connect(void) {
    ESP_LOGI(TAG_C, "Start example_connect.");
    example_wifi_start();
    ...
    // alloc mememory for password and ssid

    memcpy(wifi_config.sta.ssid, current_config.ssid, sizeof(char) * 32);
    memcpy(wifi_config.sta.password, current_config.password,
           sizeof(char) * 64);




    esp_err_t res = example_wifi_sta_do_connect(wifi_config, true);
    return res;
