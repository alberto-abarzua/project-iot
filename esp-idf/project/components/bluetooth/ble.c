...
/****************************************************************************
 *
 * This demo showcases BLE GATT server. It can send adv data, be connected by
 *client. Run the gatt_client demo, the client demo will automatically connect
 *to the gatt_server demo. Client demo will enable gatt_server's notify after
 *connection. The two devices will then exchange data.
 *
 ****************************************************************************/
...
esp_err_t get_characteristic_value(uint16_t char_handle, uint8_t **result,
    ...
    if (status == ESP_GATT_OK) {
        ESP_LOGI(GATTS_TAG, "Get characteristic value successfully.\n");
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            esp_ble_gatts_create_service(
                gatts_if, &gl_profile_tab[PROFILE_A_APP_ID].service_id,
        ...
        case ESP_GATTS_WRITE_EVT: {
            ...
            if (!param->write.is_prep) {
                ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, value len %d, value :",
                         param->write.len);
                esp_log_buffer_hex(GATTS_TAG, param->write.value,
                                   param->write.len);
            ...
            if (strncmp((char *)param->write.value, expected_con_init_msg, 3) ==
                ...
                // PRINT CONFIGS
                ESP_LOGI(GATTS_TAG,
                         "config.trans_layer %c config.protocol_id %d",
        ...
        case ESP_GATTS_CONF_EVT:
            ...
            if (param->conf.status != ESP_GATT_OK) {
                esp_log_buffer_hex(GATTS_TAG, param->conf.value,
                                   param->conf.len);
            }
            break;
...
void cont_mode_loop() {
    ...
    while (1) {
        ...
        if(first){
            ESP_LOGI(GATTS_TAG, "Sending START");
            esp_err_t ret = send_notify("START");
            if (ret == ESP_OK) {
                ESP_LOGI(GATTS_TAG, "Sent START");
                first = 0;
            } else {
                ESP_LOGE(GATTS_TAG, "Failed to send START");
